##!/../../bin/linux-x86_64/DRVIERNAMELOWERSHORTApp
...
#epicsThreadSleep(20)
dbLoadDatabase("$(ADDRIVERNAMESTANDARD)/iocs/DRIVERNAMELOWERSHORTIOC/dbd/DRIVERNAMELOWERSHORTApp.dbd")
adUVCApp_registerRecordDeviceDriver(pdbbase) 

#/*
# * Constructor for ADDRIVERNAMESTANDARD driver. Most params are passed to the parent ADDriver constructor. 
...
#
# ADDRIVERNAMESTANDARDConfig(const char* portName, const char* serial, int maxBuffers, size_t maxMemory, int priority, int stackSize)
# epicsThreadSleep(2)

# If searching for device by product ID put "" or empty string for serial number
ADDRIVERNAMESTANDARDConfig("$(PORT)", $(CAM-CONNECT), 0, 0, 0, 0)
epicsThreadSleep(2)
...
dbLoadRecords("$(ADDRIVERNAMESTANDARD)/db/ADDRIVERNAMESTANDARD.template","P=$(PREFIX),R=cam1:,PORT=$(PORT),ADDR=0,TIMEOUT=1")
...
set_requestfile_path("$(ADDRIVERNAMESTANDARD)/DRIVERNAMELOWERSHORTApp/Db")
